...
module.exports.getRecords = (req, res) => {
  if (!utils.cache.get('queue')) {
    const data = global.queuedb.get().value()
...
module.exports.deleteRecord = (req, res, next) => {
  ...
  req
    .checkBody('season', 'Season number is required.')
    .notEmpty()
    .isInt()
    .withMessage('Season must be a valid number')
  req
    .checkBody('episode', 'Episode number is required.')
    .notEmpty()
    .isInt()
...
module.exports.addRecord = (req, res, next) => {
  ...
  req
    ...
    .withMessage('Season must be a valid number')
  req
    .checkBody('to', 'To is required')
    .notEmpty()
    .isIntOrF()
    .withMessage('To is not a valid entry')
  req
    .checkBody('from', 'From is required')
    .notEmpty()
    .isInt()
...
module.exports.addMagnet = (req, res, next) => {
  ...
  req
    ...
    .then(result => {
      ...
      return parser.addMagnetUri(req.body.magnet, req.body.filename)
    })
    .then(() => {
      res.json({
        status: true
