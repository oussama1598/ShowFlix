...
const removeParenthesis = str =>
  (str.indexOf('(') > -1 ? str.substr(0, str.indexOf('(')).trim() : str)
...
class TvShowTime extends EventEmitter {
  ...
  watch () {
    if (!this.accessToken || !this.favList) {
  ...
  fetch () {
    ...
    return utils
      .getHtml(libarayUrl.href, true, 'GET', {}, {}, 20000, true)
      .then(res => {
        if (res.headers.get('x-ratelimit-remaining') === '0') {
          const toResetIn =
            new Date(res.headers.get('x-ratelimit-reset') * 1000).getTime() /
            1000
          ...
          return Promise.reject(
            new Error(`Rate limit exceeded Waiting for ${delay.toFixed(0)}s`)
          )
        }
      ...
      .then(body => {
        ...
        async.forEach(
          ...
          (val, callback) => {
            ...
            const { number, season_number: seasonNumber } = val.last_aired
            const show = this.favList().find(
              item =>
                item.name.toLowerCase() ===
                removeParenthesis(val.name.toLowerCase())
            )
            ...
            if (
              lastSeen.number !== number || lastSeen.season !== seasonNumber
            ) {
